...
export const createListing = async (req, res, next) => {
  try {
    ...
    // Automatically get coordinates from address (for Lifestyle Match feature)
    const fullAddress = `${data.colony}, ${data.city}`;
...
export const updateListing = async (req, res, next) => {
  try {
    ...
    // If city or colony is being updated, get new coordinates (for Lifestyle Match feature)
    const oldAddress = `${listing.colony}, ${listing.city}`;
    const newAddress = `${data.colony || listing.colony}, ${data.city || listing.city}`;
